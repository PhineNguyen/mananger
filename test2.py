...
def submit_product():
    ten_san_pham =widgets['entry_product_name'].get()
    so_luong = widgets['entry_quantity'].get()
    gia_tien = widgets['entry_price'].get()
    ...
    if ten_san_pham and so_luong.isdigit() and gia_tien.replace('.', '', 1).isdigit():
        ...
        widgets['entry_product_name'].delete(0, tk.END)
        widgets['entry_quantity'].delete(0, tk.END)
        widgets['entry_price'].delete(0, tk.END)
    else:
...
def show_products():
  canvas2.delete("all")
  canvas3.delete("all")
  widgets['label_product_name'] = tk.Label(canvas2, text="Tên sản phẩm", bg="#B1C6B4")
  canvas2.create_window(220, 50, window=widgets['label_product_name'])

  widgets['entry_product_name'] = tk.Entry(canvas2)
  canvas2.create_window(220, 90, window=widgets['entry_product_name'])
    
  widgets['label_quantity'] = tk.Label(canvas2, text="Số lượng", bg="#B1C6B4")
  canvas2.create_window(220, 150, window=widgets['label_quantity'])
    
  widgets['entry_quantity'] = tk.Entry(canvas2)
  canvas2.create_window(220, 190, window=widgets['entry_quantity'])
    
  widgets['label_price'] = tk.Label(canvas2, text="Giá tiền", bg="#B1C6B4")
  canvas2.create_window(220, 250, window=widgets['label_price'])
    
  widgets['entry_price'] = tk.Entry(canvas2)
  canvas2.create_window(220, 290, window=widgets['entry_price'])

  widgets['btn_add_product'] = tk.Button(canvas2, text="Thêm sản phẩm", command=add_product)
  canvas2.create_
...
btn_show_products = tk.Button(canvas1, text="Sản phẩm", command=show_products)
canvas1.create_window(100, 50, window=btn_show_products)
...
# create Label n Entry for quantity
